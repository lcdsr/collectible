...
function addRouteToNgModule(options) {
    ...
    return host => {
        ...
        const pagePath = (`/${options.path}/` +
            (options.flat ? '' : `${lodash_1.kebabCase(options.name)}/`) +
            `${lodash_1.kebabCase(options.name)}.module`);
        ...
        const routeLoadChildren = `${relativePath}#${lodash_1.upperFirst(lodash_1.camelCase(options.name))}PageModule`;
...
function buildSelector(options) {
    let selector = lodash_1.kebabCase(options.name);
    if (options.prefix) {
